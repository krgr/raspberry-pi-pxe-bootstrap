...
init_remote_filesystems() {
	...
	echo "console=serial0,115200 console=tty1 root=/dev/nfs nfsroot=$NAS_IP:$nas_volume/$pxe_folder/$hostname,vers=4.1 rw ip=dhcp elevator=deadline rootwait cgroup_memory=1 cgroup_enable=memory" | asroot tee "$bootfs/cmdline.txt"
